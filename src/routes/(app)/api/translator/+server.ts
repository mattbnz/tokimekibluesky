...
async function translator(text = '', to = 'ja', model = undefined) {
    ...
    if (model === 'llm') {
        payload.model = `projects/${env.GCP_PROJECT_ID}/locations/us-central1/models/general/translation-llm`;
    }

    const response = await fetch(`https://translation.googleapis.com/v3/projects/${env.GCP_PROJECT_NUMBER}:translateText`, {
        method: 'post',
        headers: {
            'Content-type': 'application/json',
            'Authorization': `Bearer ${accessToken.token}`,
            'x-goog-user-project': env.GCP_PROJECT_NUMBER,
        },
